...
# Запускаем бота в отдельном потоке, чтобы он работал вместе с Gunicorn
bot_thread = threading.Thread(target=telegram_bot.run, daemon=True)
# bot_thread.daemon = True  # Установлено через аргумент daemon=True
bot_thread.start()
print("🚀 Telegram бот запущен в отдельном потоке.")
# --- Конец изменений ---

# Запускаем Flask приложение только если запускаем скрипт напрямую
if __name__ == '__main__':
    print("🚀 Запускаем Flask приложение для локальной разработки...")
    # Замечание: `use_reloader=False` важно, чтобы поток бота не создавался дважды
